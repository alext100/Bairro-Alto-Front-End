/*  eslint-disable */
import { emojiExp, getText } from "./utils/strings";
...
const cssClassFunc = (cssClass = "", tag) => {
  ...
  return () => `<${tag}${divStart}${cssClass ? ` class='${cssClass}${iterate ? num++ : ""}'>` : ">"}`;
};
...
const setPositionsAfterSplit = (element, vars, allChars, allWords, allLines, origWidth, origHeight) => {
  const cs = getComputedStyle(element);
  const paddingLeft = getStyleAsNumber("paddingLeft", cs);
  let lineOffsetY = -999;
  const borderTopAndBottom = getStyleAsNumber("borderBottomWidth", cs) + getStyleAsNumber("borderTopWidth", cs);
  const borderLeftAndRight = getStyleAsNumber("borderLeftWidth", cs) + getStyleAsNumber("borderRightWidth", cs);
  const padTopAndBottom = getStyleAsNumber("paddingTop", cs) + getStyleAsNumber("paddingBottom", cs);
  const padLeftAndRight = getStyleAsNumber("paddingLeft", cs) + getStyleAsNumber("paddingRight", cs);
  ...
  for (i = 0; i < l; i++) {
    ...
    if (!words && chars) {
      ...
      if (node._isSplit) {
        ...
        offset = !node.nextSibling && isBeforeWordDelimiter(node.parentNode, element, wordDelimiter); // if this is the last letter in the word (and we're not breaking by lines and not positioning things absolutely), we need to add a space afterwards so that the characters don't just mash together
